...
function resolveRomInDir(dir, name) {
    if (!dir)
        return undefined;
    ...
    if (target !== path.join(dir, name))
        return undefined;
    if (!ROM_EXTENSIONS.has(path.extname(name).toLowerCase()))
        return undefined;
...
export function apply(ctx, config) {
                ...
                handler: async (req, res) => {
                    try {
                        ...
                        for (const entry of entries) {
                            if (!entry.isFile())
                                continue;
                            if (!ROM_EXTENSIONS.has(path.extname(entry.name).toLowerCase()))
                                continue;
                            try {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            if (!path.isAbsolute(target) && romsDir)
                target = path.join(romsDir, target);
